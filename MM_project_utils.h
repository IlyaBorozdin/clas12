...
std::string generateTitleForYieldPlot(int iq2, int iw, int ict) {
    // Для TGraph заголовок обычно лучше оставить пустым, 
    // так как вся информация будет в TPaveText
    return ";#varphi^{*}, rad.;Y(e^{-}p#rightarrow e^{-}#pi^{+}n)";
}

std::string generateTitleForEfficiencyPlot(int iq2, int iw, int ict) {
    return ";#varphi^{*}, rad.;#epsilon";
}